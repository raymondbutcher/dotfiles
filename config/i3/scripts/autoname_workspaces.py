...
def icon_for_window(window):
    ...
    classes = xprop(window.window, 'WM_CLASS') or []
    classes = [name.lower() for name in classes]

    titles = xprop(window.window, 'WM_NAME') or []
    ...
    for name in classes:
        ...
        if name == 'gnome-terminal-server':
            ...
            for name in titles:
                if name.endswith('claranet uk mail'):
                    return fa.icons['envelope']
                elif name.endswith('slack'):
                    return fa.icons['slack']

            return fa.icons['chrome']
